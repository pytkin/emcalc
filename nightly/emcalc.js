...
if (window.globalStorage) {
	...
	var glStorage = globalStorage.namedItem('emcalc.' + document.domain);
...
var EmCalc = {
	...
	createNode: function() {
		...
		var argNodeName = arguments[0];
		var argIsRoot = arguments[1];
		var argNodePixels = arguments[2];

		var form = document.createElement('form');
		var fieldset = document.createElement('fieldset');
		var spanName = document.createElement('span');
		var spanOpenBr = document.createElement('span');
		var spanPxs = document.createElement('span');
		var spanEq = document.createElement('span');
		var spanEms = document.createElement('span');
		var spanCloseBr = document.createElement('span');
		var spanActions = document.createElement('span');
		var inputPxs = document.createElement('input');
		var inputEms = document.createElement('input');
		var linkChild = document.createElement('a');
		var linkDel = document.createElement('a');

		$(spanName).addClass('nodeName').attr('title','Double click to edit')
		.append(argNodeName).bind('dblclick', this, this.editName);
		$(spanOpenBr).append(' { ');
		$(spanEq).append(' px = ');
		$(spanCloseBr).append(' }');
		$(inputPxs).add(inputEms).addClass('text').attr('type', 'text');
		$(inputPxs).bind('keyup', this, this.calcFields);
		$(inputEms).attr('readonly', 'readonly').addClass('readonly')
			.bind('click', this, function(e) {
				e.data.selectField(this);
			});

		$(spanEms).addClass('nodeEms').append(inputEms);
		$(spanPxs).addClass('nodePxs').append(inputPxs);
		$(spanActions).addClass('nodeActs');
		$(linkChild).addClass('nodeChild').attr('href', '#').attr('title', 'Add child')
		.bind('click', this, function(e) {
			e.data.addNode(form, 'child');
			e.preventDefault();
		}).append('[Add child]');

		if (!argIsRoot) {
			var linkSibl = document.createElement('a');
			$(linkSibl).addClass('nodeSibling').attr({'href':  '#', 'title': 'Add sibling'})
			.bind('click', this, function(e) {
				e.data.addNode(form, 'sibling');
				e.preventDefault();
			}).append('[Add sibling]');
			$(spanActions).append(linkSibl);
			$(linkDel).addClass('nodeDelete').attr({'href': '#', 'title': 'Delete this node & child nodes'}).bind('click', this, this.deleteNode).append('[Delete node]');
		}

		if (argNodePixels) {
			$(inputPxs).val(argNodePixels);
		}

		$(spanActions).append(linkChild);
		$(fieldset).append(spanName, spanOpenBr, spanPxs, spanEq, spanEms, spanCloseBr, spanActions, linkDel);
		$(form).bind('submit', function(e) { e.preventDefault(); }).append(fieldset);
